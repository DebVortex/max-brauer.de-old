...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) maxbrauer/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) maxbrauer/manage.py$(1)
endif
endef
...
create-db:
	envdir envs/$(ENV) createdb -U max-brauer.de -l en_US.utf-8 -E utf-8 -O max-brauer.de -T template0 -e max-brauer.de

create-db-user:
	psql -d postgres -c "CREATE USER \"max-brauer.de\" WITH PASSWORD 'max-brauer.de' CREATEDB;"

drop-db:
	envdir envs/$(ENV) dropdb -i -e -U max-brauer.de max-brauer.de

drop-db-user:
	dropuser -i -e max-brauer.de
...
isort:
	isort --recursive setup.py maxbrauer/ tests/
...
startapp:
	...
	mkdir -p maxbrauer/apps/$$app_name; \
	envdir envs/$(ENV) python manage.py startapp $$app_name maxbrauer/apps/$$app_name --template maxbrauer/config/app_template; \
	echo "Don't forget to add 'maxbrauer.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'maxbrauer.config/settings/common.py'!"
...
test-upload:
	...
	python -c "import webbrowser; webbrowser.open('https://testpypi.python.org/pypi/maxbrauer')"

upload:
	twine upload -s dist/*
	python -c "import webbrowser; webbrowser.open('https://pypi.python.org/pypi/maxbrauer')"
